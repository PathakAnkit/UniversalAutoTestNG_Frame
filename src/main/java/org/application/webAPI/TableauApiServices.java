...
import org.application.tableauBindings.CapabilityType;
import org.application.tableauBindings.FileUploadType;
import org.application.tableauBindings.GranteeCapabilitiesType;
import org.application.tableauBindings.GroupType;
import org.application.tableauBindings.ObjectFactory;
import org.application.tableauBindings.PermissionsType;
import org.application.tableauBindings.ProjectListType;
import org.application.tableauBindings.ProjectType;
import org.application.tableauBindings.SiteListType;
import org.application.tableauBindings.SiteType;
import org.application.tableauBindings.TableauCredentialsType;
import org.application.tableauBindings.TsRequest;
import org.application.tableauBindings.TsResponse;
import org.application.tableauBindings.WorkbookListType;
import org.application.tableauBindings.WorkbookType;
...
public class TableauApiServices {
    ...
    // The only instance of the TableauApiServices
    private static TableauApiServices INSTANCE = null;
    ...
    /**
     * Initializes the TableauApiServices if it has not already been done so.
     *
     * @return the single instance of the TableauApiServices
     * @throws IOException 
     */
    public static TableauApiServices getInstance() {
        if (INSTANCE == null) {
            INSTANCE = new TableauApiServices();
            initialize();
        }
    ...
    private static UriBuilder getApiUriBuilder() {
        return UriBuilder.fromPath(m_properties.getProperty("server.host") + "/api/3.10");
    }
    /**
     * Initializes the TableauApiServices. The initialize code loads values from the configuration
     * file and initializes the JAXB marshaller and unmarshaller.
     */

	private static void initialize() {
        try {
        	m_properties.load(new FileInputStream((System.getProperty("user.dir") + "\\src\\test\\resources\\Configurations\\config.properties")));
            ...
            throw new IllegalStateException("Failed to initialize the REST API "+ex.getMessage());
        }
    }
    ...
    private Logger m_logger = Logger.getLogger(TableauApiServices.class);
    ...
    private TsResponse unmarshalResponse(String responseXML) {
        ...
        try {
            ...
            m_logger.error("Failed to parse response from server due to: "+e.getMessage());
            e.printStackTrace();
        } 
